     ...
     '(#:phases
       (modify-phases %standard-phases
         (add-after 'unpack 'generate-cmake-file
           (lambda* (#:key inputs outputs #:allow-other-keys)
             (let ((autocmake-path (assoc-ref inputs "autocmake-src")))
           ...
           (lambda* (#:key inputs outputs #:allow-other-keys)
             (substitute* "src/CMakeLists.txt"
               (("\\$\\{PROJECT_SOURCE_DIR\\}/external/upstream/taylor")
                (search-input-directory inputs "include/taylor"))))))))
    (home-page "https://github.com/dftlibs/xcfun")
    (synopsis "Library of exchange-correlation functionals with automatic
differentiation")
    (description "@code{XCFun} is a library of exchange-correlation
functionals with arbitrary-order derivatives for density functional theory.")
    (license license:mpl2.0)))
...
(define-public libcint
  (package
    ...
    (description "@code{libcint} is a C library (also with a Fortran API) to
evaluate one- and two-electron integrals for Gaussian type functions.")
    (license license:bsd-2)))
...
(define-public python-geometric
  (package
    ...
    (propagated-inputs
     (list python-numpy python-scipy python-networkx))
    (arguments
     '(#:phases
       (modify-phases %standard-phases
         (replace 'check
...
(define-public mctc-lib
  (package
    ...
    (description "@code{mctc-lib} (modular computation tool chain library) is
